...
\indent 如果你知道原来的位置是在之前的位置之上还是之下, 你需要设置 \inlinepython{coming_from} 参数. 当 \inlinpython{ListBox} 完成偏移量和百分比的设置后, 它会尝试去寻找原来的控件. 如果原来的控件仍然是可见的, 它将试图避免引发 \inlinpython{ListBox} 的内容从原来的位置向上或向下滚动. 如果控件是不可见的, 那么 \inlinpython{ListBox} 将:%
%
\begin{itemize}
  \item 在 \inlinpython{ListBox} 的底端显示焦点控件, 如果参数 \inlinepython{coming_from} 的值表示之上;
  \item 在 \inlinpython{ListBox} 的顶端显示焦点控件, 如果参数 \inlinepython{coming_from} 的值表示之下;
  \item 在 \inlinpython{ListBox} 的中间显示焦点控件, 如果参数 \inlinepython{coming_from} 的值为 \inlinepython{None};
\end{itemize}%
%
