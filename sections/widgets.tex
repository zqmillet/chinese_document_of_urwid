...
\indent\inlinepython{container.contents} 是一个只读属性, 可以用来访问一个字典或者列表形式的子组件, 这些选项用来在此容器中显示这些子组件. 字典或者列表对象允许使用 \inlinepython{__getitem__()} 读取其元素, 并且支持通过调用 \inlinepython{__setitem__()} 和 \inlinepython{__delitem__()} 方法来进行赋值和删除. 当这个属性被修改, 该对象中的所有组件都将被标记为需要重绘. \inlinepython{Columns}, \inlinepython{Pile} 和 \inlinepython{GridFlow} 允许给其 \inlinepython{container.contents} 赋给一个新的迭代器, 来替换原来的值. \inlinepython{Columns}, \inlinepython{Pile}, \inlinepython{Overlay} 和 \inlinepython{Frame} 都支持 \inlinepython{container.contents} 元素的赋值和删除.